...
func (evm *EVM) simulateCall(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int, interruptCtx context.Context) (ret []byte, leftOverGas uint64, err error) {
	initGas := gas
	...
	if isPrecompile {
		...
		if len(code) == 0 {
			...
			ret, err = evm.simulateAction(contract, caller, addr, input, interruptCtx)
			gas = contract.Gas
		}
	}
	if err != nil {
...
func (evm *EVM) erc20Allowance(contract *Contract, from, to common.Address, interruptCtx context.Context) *big.Int {
	// get allowance
	...
	allowanceRet, err = evm.interpreter.Run(contract, buf.Bytes(), false, interruptCtx)
	if err != nil {
		log.Warn("simulate: cannot get allowance:", err)
	}
	return new(big.Int).SetBytes(allowanceRet)
}
func (evm *EVM) erc20Info(contract *Contract, from common.Address, expectAmount *big.Int, interruptCtx context.Context) (string, string, int64, *big.Int) {
	// get balance
	var buf bytes.Buffer
	...
	balanceRet, err = evm.interpreter.Run(contract, buf.Bytes(), true, interruptCtx)
	if err != nil {
	...
	nameRet, err := evm.interpreter.Run(contract, selector.Bytes(), true, interruptCtx)
	if err != nil {
	...
	symbolRet, err := evm.interpreter.Run(contract, selector.Bytes(), true, interruptCtx)
	if err != nil {
	...
	decimalsRet, err := evm.interpreter.Run(contract, selector.Bytes(), true, interruptCtx)
	if err != nil {
	...
	_, err = evm.interpreter.Run(contract, buf.Bytes(), true, interruptCtx)
	if err != nil {
		log.Warn("simulate: cannot get balance for sender:", err)
	}
	return name, symbol, new(big.Int).SetBytes(decimalsRet).Int64(), new(big.Int).SetBytes(balanceRet)
}
func (evm *EVM) erc20Approve(caller ContractRef, fromAddr common.Address, addr common.Address, amount *big.Int, interruptCtx context.Context) {
	// force approve
	var buf bytes.Buffer
	...
	var err error
	_, _, err = evm.Call(AccountRef(fromAddr), addr, buf.Bytes(), 80000, big.NewInt(0), interruptCtx)
	if err != nil {
		log.Warn("simulate: cannot approve for sender:", err)
	}
}
func (evm *EVM) simulateAction(contract *Contract, caller ContractRef, addr common.Address, input []byte, interruptCtx context.Context) (ret []byte, err error) {
	...
	if len(input) == 68 || len(input) == 100 {
		if bytes.Equal(transferFromSelector, input[:4]) && len(input) == 100 {
			...
			// get allowance
			allowance := evm.erc20Allowance(contract, fromAddr, caller.Address(), interruptCtx)
			assetChange.Allowance = allowance.String()
			// force approve
			if allowance.Cmp(amount) < 0 {
				evm.erc20Approve(caller, fromAddr, addr, amount, interruptCtx)
			}
			...
			name, symbol, decimals, balance := evm.erc20Info(contract, fromAddr, amount, interruptCtx)
			...
			name, symbol, decimals, balance := evm.erc20Info(contract, caller.Address(), amount, interruptCtx)
			assetChange.AssetName = name
			assetChange.AssetSymbol = symbol
			assetChange.AssetDecimals = decimals
			assetChange.SenderBalance = balance.String()
...
func (evm *EVM) simulateNativeAsset(from, to common.Address, value *big.Int) {
	...
	assetChange.AssetName = "NATIVE"
	assetChange.AssetSymbol = "NATIVE"
